...
router.post('/applications/:id/approve-confirm', async (req, res, next) => {
  try {
    ...
    const docTriggerStatus = await documentTriggerService.triggerDocumentGeneration(id, {
      ...
      // DB-sourced (formFields may override below)
      role: formFields.role || formFields.offer_title || app.position_title,
      ...
      signatory: formFields.signatory || 'Dr. Alex Vance',
      signatory_role: formFields.signatory_role || 'Director of Engineering',
      signatory_title: formFields.signatory_title || formFields.signatory_role || 'Director of Engineering',
      signature_text: formFields.signature_text || formFields.signatory || 'Dr. Alex Vance',
      // offer_letter_v2.html fields
      start_date: formFields.start_date || formFields.date || new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }),
      end_date: formFields.end_date || new Date(Date.now() + 180 * 24 * 60 * 60 * 1000).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }),
      date: formFields.date || new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }),
      duration: formFields.duration || '6 Months',
      engagement: formFields.engagement || 'Internship',
      reporting_to: formFields.reporting_to || formFields.signatory || 'Dr. Alex Vance',
      work_mode: formFields.work_mode || 'Remote',
      address: formFields.address || 'Remote / India',
...
router.post('/applications/:id/issue-certificate-confirm', async (req, res, next) => {
  try {
    ...
    const docTriggerStatus = await documentTriggerService.triggerDocumentGeneration(id, {
      ...
      signatory_title: formFields.signatory_title || formFields.signatory_role || 'Director of Engineering',
      signature_text: formFields.signature_text || formFields.signatory || 'Dr. Alex Vance',
      // certificate_of_compleation.html + offer_letter_v2.html fields
      start_date: formFields.start_date || formFields.date || new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }),
      end_date: formFields.end_date || new Date(Date.now() + 180 * 24 * 60 * 60 * 1000).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }),
      date: formFields.date || new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }),
      status: formFields.status || 'SUCCESSFULLY COMPLETED',
      program_lead: formFields.program_lead || formFields.signatory || 'Dr. Alex Vance',
      program_lead_org: formFields.program_lead_org || formFields.organization_name || 'Code Plus Academy',
      program_lead_title: formFields.program_lead_title || formFields.signatory_role || 'Director of Engineering',
      engagement: formFields.engagement || 'Internship',
      reporting_to: formFields.reporting_to || formFields.signatory || 'Dr. Alex Vance',
      work_mode: formFields.work_mode || 'Remote',
      address: formFields.address || 'Remote / India',
